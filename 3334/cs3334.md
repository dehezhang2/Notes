...
* Delete : need to rebalance => ugly code => splay trees
...
* Splay trees
  ...
  * Zig : rotate once, Zag ; rotate twice

  * Find : Find the node in normal BST manner • Splay the node to the root

  * Split : divide the tree by arbitrary key(may not be root), divide it into 2 trees
