...
* Pre-centering: Under the assumption that the data are pre-centered so that ${1\over m}\sum^m_{i=1}\textbf v^T\textbf x^{(i)}={1\over m}\sum^m_{i=1}\textbf x^{(i)}=0​$, this expression simplifieds to: 
  $$
