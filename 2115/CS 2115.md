...
* Circuit Simplification
  ...
  * Binary Ripple Carry Adder
    ...
    * avoid overflow (judge)
      ...
      - Also can be detected by checking whether the carry bits of last bit and second last bit are same
  ...
  * Carry-Lookahead Adder
    ...
    * method
      ...
      * Expand the equation
        ...
        * all carries can be obtained three gate delays after applying input A,B and $C_0$ 
...
* Relative addressing mode : Effective address = base + offset(often in PC register) => move from base
...
## Let 08 Central Processing Unit
...
* Single bus Organization
  ...
  * Instruction decoder and control logic : Implementing the actions specified by the instruction loaded in IR by
    * ==issuing signals== that control the operation of all the unit inside the processor
    * interacting with the memory bus via *control* lines
    ...
    * **datapath** : Registers, ALU and the interconnecting bus
...
* Functions of the CPU
  ...
  * Fetch(or store) data in(or to) memory(address stored in R1) and load into(or write from) the register(R2). Suppose $MAR_{out}$ is always on
...
* A complete instruction : to add the content of a memory location pointed to by R3 to R1
  ...
  * First 3 steps is to load the instruction(Fetch phase), in this example, the loaded instruction is ADD, it is decoded by control unit connected to the IR
...
* Address : Reference number of each cell, refered by whcih program can refer to it
...
* Ways to **read** from cache (mapping the address in main memory with the data block stored in the cache)
  ...
  * Set-associative mapping
    ...
    * 4-block cache example
      ...
      ![](屏幕快照 2018-11-22 下午11.26.28-2901341.png)
