...
* Measurment
  ...
  * For only one of the transmitted

- Sender :

  - ACK(n) $\color{red}{all\ byte\ before\ n\ }$ is correctly received (window remove to the expected one)
  - Single timer
  - Retramsmssion (bad thing of go-back-n)
    - 2 trigger of retransmission
      ...
      - duplicate ACK => segment is lost, a lot of segment after it received by receiver, then receiver will send back a lot of same ACK
        - if duplicate ACK exist, there might be error, but also possible to be correct => thus receive 3 ACKs for same data then resend unacked segment with smallest seq#
...
- Receiver
  ...
  - loss or corrupted ACK : receiver will just send expected
    ...
    | Event                                                        | Action                                                       |
    | ------------------------------------------------------------ | ------------------------------------------------------------ |
    | arrival correct seq# pkt, and all pkt are checkedsum and correct | Delayed ACK : wait for the next segment for about 500ms      |
    | correct seq#, one other segment is being checked sum         | Immediately send single cumulative ACK, ACKing both in-order segments |
