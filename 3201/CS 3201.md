...
* Error detection
  ...
  * Cyclic redundanct check (any number of error=>depends on redundant)
    ...
    * Goal : choose r CRC bits $R$, s.t. 
      ...
      * can detect all burst error less than $r+1$ bits (**why r+1 maintains a $\color {red} {at \ most}$ r bits remainder**?)
        * r+1 bits => D << r has r '0' and the end, when do the division, there are only 2 cases
          ...
          * Last r+1 : 1000...00 => remainder less than r+1 digiits
...
* Multiple access links protocols(access control)
  ...
  * two or more simultaneous transmissions by nodes: interference (干涉)
...
* Pure ALOHA (no time slots)
  ...
  * If data doesn’t get through (receiver sends acknowledgement) then it retransmit with probability p, if it fail to retransmit(1-p), then retransmit after a ==random== delay
...
* CSMA/CD (collision detection) : keep on listen and special back off after collision
  ...
  * Procedure
    ...
    * After aborting, NIC enters **binary(exponential) backoff** (waiting time) :
      ...
      * Slots can be defined by **Prof.** zlq as : a transmission time (i.e. $frame\ size \over bandwidth$ the transmission time for one frame) 
  ...
  * Minimum Ethernet Frame size (needed under the collision detection system)
    ...
    * To send a collision message : The longest time between start to transmit a frame and receiving the first bit of a jam sequence(collision message) is **twice the propagation delay** from one end to another
      ...
      * Other side send collition message(treated as 1 bit) : 1 propagation delay
    ...
    * Thus the length of each frame must be long enough to hold on for 2 propagation delay (else when the message goes back, the frame is already finished and dequeued) 
      $$
      2*t_{prop}={frame\ size \over transmission\ speed(bandwidth)}
      $$
