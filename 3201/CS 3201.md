...
* **Interface between the process and the computer network**
  ...
  * A process sends messages into or receives messages from the network through a software interface called **socket** (also called **API** between applications and network)
  * Control by the aoolication developer
...
* On the receiver side, transport layer send the message to the application layer
  ...
  * HTTP : default: persistent connection; runnable : both non-persistent and persistent
    * Non-persistent connection=>each request sent over a *separate* TCP connection
      * steps
        ...
        * repeat the steps above until get all the objects
