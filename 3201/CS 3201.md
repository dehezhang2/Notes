...
* Web cache
  ...
  * Browser sends all HTTP requests to cache
    ...
    * Otherwise the cache requests the object from the origin server, then returns the object to the client
  * If origin updated
...
* DNS : domain name system
  ...
  * Internet hosts, routers:
    ...
    * domain "Name" => used by human

  * DNS : 

    * distributed database implemented in hierarchy of many name servers : to avoid attack(if it is save in a single one, will be attacked) => block chain to avoid attack
    ...
    * root name servers:
      ...
      * Local DNS name server
        ...
        * not the hierarchy but it is the one who send the query
