...
* ISPs : Internet service provider => provide internet-access service to end systems
  - Tier1: national (also interconnect(peer) privately)

  - Tier2: regional => connect to 1 or more Tier1 (also peer privately with each other)
...
* **Interface between the process and the computer network**
  ...
  * Address processes : for client to match message returned and process or server to match process and coming message
    ...
    * for e.g. the port 80 is always for HTTP server (only on server side)
...
* Application-layer protocol define how an application's processes, running on different end systems, pass messages to each other. It defines:
  ...
  * Rules for determining when and hows a process sends messages and responds to the message (e.g. HTTP, DNS，Skype )
...
* Request message (five lines)
  ...
  * The first line : **request line** => 3 fields
    ...
    * HTTP version field
  * 2th : **header line**
    * specifies the host on which the object resides(may stored in the web cache)
...
* Web cache
  ...
  * If origin updated (The conditional GET)
    ...
    * Goal : don’t send the object if cache has up-to-date version 
...
* **Subnet** : Network interconnecting three host interfaces and one router interface forms a subnet
  ...
  * IP definition of a subnet is $\color{red}{not}$ restricted to Ethernet segments that connect multiple hosts to a router interface.
...
* **Classless Interdomain Routing** (CIDR)
  ...
  * The network portions of an IP address were constrained to be 8, 16, or 24 bits in length, an addressing scheme known as **classful addressing** (before teh CIDR is invented)
    ...
    * get the prefix and remain
      ...
      ![](屏幕快照 2018-10-16 下午10.34.19.png)

      ![](屏幕快照 2018-10-16 下午10.34.24.png)
...
* Obtaining a Block of Addressses => for subnetwork
  ...
  * Method 1
    ...
    * In order to obtain a block of IP addresses for use within an organization's subnet, a network administrator might first contack its ISP, which would provide addresses from a larger block of addresses that had already been allocated to the ISP
...
* Network Address Translation (NAT)
  ...
  * A **realm** with private addresses refers to a network whose IP addresses only have meaning to devices within that network
  ...
  * **NAT translation table** => include port numbers as well as IP addresses in the table entries
    ...
    * The host (says a PC) ask for a Web server (port 80) with IP address 128.119.40.186 .
    ...
    * The Web server doew know the port has been manipulated by the NAT route, responds with a datagram whose destination address is the IP address of the NAT router
...
* *Dijkstra's algorithm* 
  ...
  * Define the notation:
    ...
    * N􏰘 : subset of nodes; v is in N􏰘 if the least-cost path from the source to v is defin- itively known. 
  ...
  * Example
    ...
    * After running the LS algorithm, all nodes store the least-cost path from itself to a series of destinations, so the forwarding table just store the next-hop node on the least-cost path from current router to the destination (u remembers x, x remembers y,if the target is z)
...
* A comparison of LS and DV Routing Algorithm
  ...
  * Message complexity: 
    ...
    * DV algorithm depends on many factors (change of link-cost etc.)
  * Speed of convergence(收敛)
    * LS converge when the $O(N^2)$ program finished
    * DV converge only when there is no change of link cost(also count-to-infinity problem)
  * Robustness(稳健，强)
    ...
    * DV: A node can advertise incorrect least-cost path to ==all== destinations
...
* Solution : Organizing routers into **autonomous systems(ASs)** , with each AS consisting of a group of routers that are typically under the same administrative control (Routers in a same AS will run the same routing algorithm and have information about each other) 
  ...
  * Source router and destination router are not in the same AS
    ...
    * if there are 2 or more links of the AS (**inter-AS routing protocol**) 
      ...
      * AS1 need to learn 
        * Which destinations are reachable via AS2  and which dests are reachable via AS3
...
* RIP: Routing Information Protocol (based on DV)
  ...
  * difference of the forwarding table using this method : need to maintain cost (hops), and the cost may be changed by the change of the network
...
* BGP(boarder gateway protocol) : two BGP routers (“peers”) exchange BGP messages
  ...
  * when AS3 advertises a prefix to AS1:
    ...
    * AS3 can aggregate(统计) prefixes in its advertisement
...
* Do In-network duplication 
  ...
  * controlled flooding: **node only broadcasts pkt if it hasn’t broadcast same packet before**
    * node keeps track of packet ids already broadacsted (buffer the sender packet => to busy of router)
  ...
  * Spanning tree (go through every nodes only once => make the graph has no cycle)
    * only send packets to nodes not on the spanning tree
    * Creation
      * center node
      * each node sends unicast(one-to-one) **join** message to center node
      * message forwarded until it arrives at a node already belonging to spanning tree (DFS)

#### 4.7.2 Muticast Routing Algorithms
...
* IGMP messages:
  ...
  * Leaving a group 
    * leave report
  * Maintenance 
    * query (general : all multicast group, special : specific multicast group)
    * membership report 
* Joining
  ...
  * Local routers adds this group if it was not in database of active groups (only send the first request)(the source also has a database of router who has the interested host), and propagates appropriate messages to other routers
* Leaving
...
* Membership Monitoring 
  ...
  * Router just expects the response message from each group (1000 hosts interest in 100 groups in total under a router => expect 100 messages)
  * Hosts **delay response to reduce traffic** (max response time for query 100msec)
    * ==Hosts== wait a **random time** between zero and maximum response time
      * maintains a timer for each group to waiter for response of general query message
    * If it sees another response while waiting (responses are broadcast), it does NOT send response (cancels its timer)
* IP **multicast** routing
  ...
  * Distribution tree structure
    ...
    * shared tree : specify some node to be root
      * root unicast to the shared tree's root
...
* Error detection
  ...
  * EDC : Error Detection and Correction bits (redundancy) => only detect error, if occurs, throw away the frame, retransmission is taken care by receiver
  ...
  * 2-dimensional bit parity
    ...
    * cannot check all 4-bits error 

  * Cyclic redundanct check (any number of error=>depends on redundant)
    ...
    * Goal : choose r CRC bits $R$, s.t. 
      ...
      * (D << r $\bigoplus$  R) % G = 0 (redefine the plus operation)
      * **In the redefined calculation, addition and substraction can be treated as a same operation (because there is no carrying bit)**
      * receiver knows G, divides D << r $\bigoplus$  R by G, if non-zero remainder: error detected
      * can detect all burst error less than $r+1$ bits (**why r+1 maintains a r bits remainder**?)
        * r+1 bits => D << r has r '0' and the end, when do the division, there are only 2 cases
          * last 4 : 0000 => remainder less than r+1 digits
          * Last 4 : 1000 => remainder less than r+1 digiits

    * Proof
      $$
      (D<<r)\oplus R=n*G\\
      (D<<r)\oplus R\oplus R=n*G\oplus R\\
      (D<<r) = n*G\oplus R
      $$
    ...
    * polynomial representing a divisor (x equals to 2)
      ...
      ![](屏幕快照 2018-11-14 上午10.12.10.png)

* Multiple access links protocols

  * two or more simultaneous transmissions by nodes: interference 
    * collision if node receives two or more signals at the same time
  * Broadcast : shared wire or medium , only 1 source can tranfer at one time
...
* MAC protocol : 
  ...
  * "taking turns"
    ...
    * But if someone get turn, it will hold it for a long time, others may wait for a long time (different with channel partitioning, it doesn't define when to give turn to others)
...
* Slotted ALOHA
  ...
  * assumptions
    ...
    * nodes are synchronized (timer are same)
    * if 2 or more nodes transmit in a slot, all nodes detect collision (in hardware perspective, 2 or more transmitting has different signal with 1, thus can be detected)
  * Operation
  ...
  * Cons : 
    ...
    * nodes may be able to detect collision earlier than the time to transmit packet (possible optimization )
    * difficult to implement clock synchronization

* Pure ALOHA

  * Whenever a user has a frame to send, it simply transmits the frame
  * If data doesn’t get through (receiver sends acknowledgement) then retransmit after
    a ==random== delay
...
* Carrier Sense Multiple Access (CSMA)
  ...
  * Procedure
    ...
    *  During sending, keep on sensing, if something wrong just stop
  * Advantage : 
...
* CSMA/CD (collision detection)
  ...
  * Procedure
    * NIC sense chanel idle, starts frame transmission, otherwise wait until channel idle
    ...
    * After aborting, NIC enters **binary(exponential) backoff** (waiting time) :
      * after $m^{th}$ collision, NIC chooses K at random from $\{0,1,2, …, 2^m-1\}$. NIC waits $K$ **slots**, returns to Step 1
      * Slots can be defined by **Prof.** zlq as : a transmission time (i.e. $frame\ size \over bandwidth$) 
...
* Minimum Ethernet Frame size
  ...
  * To send a collision message : The longest time between start to transmit a frame and receiving the first bit of a jam sequence(collision message) is **twice the propagation delay** from one end to another
    ...
    * Other side send collition message : 1 propagation delay
...
* Wireless network
  ...
  * Elements:
    ...
    * wireless link : 
      ...
      * muyltiple access protocol coordinates link access
  ...
  * problem
    ...
    * $\color{red}{Hard\ to\ detect\ collision}$ in wireless networks
    ...
    * Hidden and Exposed station problem
      ...
      * range of a single radio may not cover the entire system
        ...
        ![](屏幕快照 2018-11-25 下午4.47.49.png)

      * If we apply CSMA, collision will happen a lot

        * A->B, C->B since C does not hear A,  collision
        * Hidden station problem  (C hiden from A)

      * If we apply CSMA directly, efficiency may not be high

        * **B->A**, C hears B, C won’t send to D, reduced efficiency
        * Exposed station problem (C exposed to B)

  * Solution : CSMA/CA in Wireless Communication
    ...
    * In the second case, if A->B, C cannot send or receive to D, why ?
      ...
      * Because the signal transfered to all the direction since it is wireless, C will affect B if it is not silent
    ...
    * Disadvantage : 
      ...
      * lack of efficiency comared to CSMA-CD
  ...
  * Random Contention Access(also DCF => distributed coordination function)
    ...
    * Choose DCF contention window
      ...
      * optimal cw for known number of contenders & know packet size
        ...
        * Tricky to implement because number of contenders is difficult to estimate and can be VERY dynamic =>still difficult to deal with when selected random numbers are the same => binary exponential backoff in DCF
...
* Taking turns protocol
  ...
  * types
    ...
    * polling (轮询)
      ...
      ![](屏幕快照 2018-11-25 下午6.00.27.png)

    * token passing (transmit permision)

      ![](屏幕快照 2018-11-25 下午6.03.54.png)

* MAC address and ARP(address resolution protocol)

  * recall IP address : network-layer address for interface
  ...
  * MAC (or LAN or physical or Ethernet)address : 
  ...
  ![](屏幕快照 2018-11-25 下午6.10.34.png)

  ![](屏幕快照 2018-11-25 下午6.10.58.png)
  ...
  * ARP: address resolution protocol
    ...
    * ARP table : each node(host, router) on LAN has table
      ...
      * IP/MAC address mappings for some LAN nodes 
        ...
        $\color {red} {< IP\ address; MAC\ address;TTL>}$
